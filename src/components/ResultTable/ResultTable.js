...
export default function ResultTable(props)  {
		...
		<thead>
			<tr>
				<th>№</th>
				{column.map(e => <th>{e}</th>)}
			</tr>
		</thead>
		<tbody>
			{rows.map( (e,i) => <CreateRows row={e} index={i} obj={objs[i]}/>)}
		</tbody>
	</table>
...
function CreateRows(props)  {
	const {row, index, obj}  = props
		...
		const handlerMouseOver = () => {
			console.log(activeWhenHover.current)
	...
	return (
		...
		<tr className="resultTable_obj-hover" >
			<td ref={hover} onMouseOut={handlerMouseOut} onMouseOver={handlerMouseOver}>{index + 1}</td>
			{row.map( (e) => <td ref={hover} >{e}</td>)}
			<div className="resultTable_obj" id='resultTable_obj' ref={activeWhenHover}>{obj}</div>
		</tr>
